...
- iOS/macOS app — read + write-back a user-chosen synced folder (issue #104): the SwiftUI app can now point at a folder your devices sync (an iCloud Drive / Syncthing-managed `~/fond`) instead of only its private sample sandbox, closing the phone-in-sync-with-your-Mac loop. Open **Settings ▸ Recipe Storage** (sidebar gear; ⌘, on macOS) to choose a folder; it's persisted as a **security-scoped bookmark** (the scope is held open for as long as the app is bound to it) so the choice survives relaunch. The app treats the folder as your fond home (ADR-002): edits, new recipes, deletes, and attached photos are written there as `.cook`/image files using the same **atomic write-temp-then-rename** as the CLI, so a sync daemon never sees a half-written file and `fond reindex` picks the changes up. It **reindexes on external change** (a coalesced `recipes/` folder watcher) and on returning to the app, so edits made on another device appear without a manual refresh. First launch keeps today's behavior — bundled samples seeded into the sandbox — and switching to a synced folder never seeds into your own collection; **Use Sample Library** switches back, leaving your files untouched. macOS reaches the same picker/bookmark path for parity, and a new sandbox + user-selected-files entitlement is added for signed builds (inert on the unsigned local PoC). No Rust changes — the existing `fond-ffi` write surface already targets a data dir. Docs: a new "Using the app with a synced folder" section (recommended iCloud/Syncthing setup, why to keep `fond.db` out of sync, and concurrent-edit limitations). Overlay (notes/ratings) write-back remains a follow-up. See [ADR-011](docs/adr/011-native-apple-bridge.md)
...
- Encryption at rest for the authored-overlay sidecar (issue #103, ADR-019): the personal **authored overlay** — notes, ratings, cook logs, dietary profiles, pantry, and meal plans — can now be exported as a single **sealed, authenticated bundle** instead of plaintext JSONL, so it can cross an untrusted file-sync channel confidentially. `fond overlay export --encrypt` writes `overlay/authored-overlay.fenc` sealed with **XChaCha20-Poly1305**; the key is **user-controlled** — a random 32-byte key stored in the **OS keychain** (default, reusing the existing `keyring-core` credential store) or one **derived from a passphrase with Argon2id** (`--passphrase`, or `FOND_OVERLAY_PASSPHRASE` for headless/CI use) — and is **never hardcoded or fetched from the network**. Import is transparent — `fond overlay import` detects the sealed bundle, reads its key mode from a self-describing header, and decrypts — and `fond overlay status` reports whether the overlay is encrypted and which key mode. Everything **fails closed**: a missing/wrong key or a tampered/truncated bundle makes import **error out and write nothing** (no silent plaintext fallback), and `--encrypt` refuses to emit a plaintext export when it cannot obtain a key. The envelope header (magic, version, key mode, Argon2 parameters, nonce) is authenticated as associated data, so tampering is detected. `fond reindex` stays **non-interactive**: it silently decrypts keychain-keyed bundles but **skips passphrase-keyed bundles with a hint** rather than blocking on a prompt. Plaintext JSONL remains the default (line-diffable); encryption is strictly **opt-in** and trades diffability for confidentiality. Baseline at-rest protection for `fond.db`, `.cook` files, and photos is **delegated to OS full-disk encryption** (FileVault / LUKS / BitLocker) and documented honestly, along with the threat model and limitations, in the new **"What fond protects"** guide. Both codecs funnel through one shared merge engine, so ADR-015 merge semantics are unchanged. New platform-free `fond-store::crypto` module; new `chacha20poly1305`, `argon2`, `zeroize`, and `rpassword` dependencies; **no new migration and no CI change** (the single `CI` gate is unaffected). See [ADR-019](docs/adr/019-encryption-at-rest.md)
