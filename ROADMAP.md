...
- Multi-recipe meal-time coordination (oven/stove contention solver); community recipe sharing (opt-in, ownership-preserving); voice cook mode.
  ...
  - **Voice cook mode — delivered** ([ADR-018](docs/adr/018-voice-cook-mode.md)): `fond cook <slug> --voice` is a hands-free cook surface that reads steps aloud and is driven by spoken (or typed) commands — step navigation ("next", "back", "repeat", "what's next", "go to step three"), ingredient queries ("how much butter?", "list ingredients"), and timers ("set a timer for ten minutes", "start/stop timer", "how much time is left"), announcing firing timers aloud. On-device speech is the default (`say`/`spd-say`/`espeak`/`System.Speech`) with graceful fallback to on-screen text; speech-to-text is pluggable (`--listen-cmd`, or pipe a recognizer), and any external/cloud engine is opt-in and clearly labeled — nothing requires the network (principles #1 / §6). Pure, exhaustively-tested `fond-voice` grammar + cook-state brain; single recipe. Noisy-kitchen recognition accuracy `[Validation Required]`.
